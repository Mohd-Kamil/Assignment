...
const Dashboard: React.FC = () => {
  ...
  const fetchNotes = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('https://assignment-jv6e.onrender.com/api/notes', {
  ...
  const handleCreateNote = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        'https://assignment-jv6e.onrender.com/api/notes',
        { content: noteContent },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setNoteContent('');
  ...
  const handleDeleteNote = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://assignment-jv6e.onrender.com/api/notes/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchNotes();
    } catch (err: any) {
